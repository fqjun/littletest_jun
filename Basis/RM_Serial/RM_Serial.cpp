...
int16_t SerialPort::_angle_reduction = 0x0000;
int16_t SerialPort::_acceleration_reduction = 0x0000;
...
void SerialPort::RMreceiveData(unsigned char *output)
{
    ...
    {
        output[i] = g_rec_buf[i]; //TODO：测试去除-0的效果
    }

    // 解码 g_rec_buf_temp 中的信息
    // _angle_reduction = mergeIntoBytes(g_rec_buf[4], g_rec_buf[5]);
    // _acceleration_reduction = mergeIntoBytes(g_rec_buf[7], g_rec_buf[8]);
    // _angle_reduction = (g_rec_buf[4] << 8) | (g_rec_buf[5]);
    // _acceleration_reduction = (g_rec_buf[7] << 8) | (g_rec_buf[8]);

    // output[0] = (g_rec_buf[0] /* - '0' */);  // S TODO：测试去除-0的效果
    // output[1] = (g_rec_buf[1] /* - '0' */);  // 颜色
    // output[2] = (g_rec_buf[2] /* - '0' */);  // 模式
    // output[3] = (g_rec_buf[3] /* - '0' */);  // 机器人ID
    // output[4] = _angle_reduction;            // 角度
    // output[5] = (g_rec_buf[6]);              // 角度与 π 的倍数
    // output[6] = _acceleration_reduction;     // 角加速度
    // output[7] = (g_rec_buf[9] /* - '0' */);  // E

    // _angle_reduction = 0x0000;  //TODO:测试上述转换方法可行之后看看是否可以去除
    // _acceleration_reduction = 0x0000;
 ...
 *           Wcjjj
 */
void SerialPort::RMserialWrite(const int &_yaw, const int16_t &yaw, const int &_pitch, const int16_t &pitch, const int16_t &depth, const int &data_type, const int &is_shooting)
{
    ...
    */
...
#if SHOW_SERIAL_INFORMATION == 1
    _yaw_reduction = mergeIntoBytes(g_write_buf[5], g_write_buf[4]); //TODO:测试传出的值是否正确
    ...
    cout << "g_write_buf=  " << g_write_buf[0]
         ...
         << "  " << static_cast<int>(g_write_buf[11])
         << "  " << g_write_buf[12] << endl;
#else
    cout << "g_write_buf=  " << g_write_buf[0]
         << "  " << static_cast<int>(g_write_buf[1])
